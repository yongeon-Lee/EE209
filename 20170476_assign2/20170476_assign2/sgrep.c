...
DoReplace(const char *pcString1, const char *pcString2)
{
  ...
  char buf[MAX_STR_LEN*MAX_STR_LEN+2];
  char *tmp = NULL;
  char *flag = NULL;
  char *state = NULL;
  char *save = (char *)malloc(2*MAX_STR_LEN+2);
  if(len1 > MAX_STR_LEN){
    fprintf(stderr,"Error: argument is too long\n" );
    return FALSE;
  }
  ...
  while(fgets(buf,sizeof(buf),stdin)){
    ...
    state = buf;
    while(TRUE){
      if((flag=StrSearch(state,pcString1))!=NULL){	
	tmp=flag+len1;
	StrCopy(save,pcString2);
	StrCopy(flag,StrConcat(save,tmp));
	state = flag+len2;
	continue;
      }
      else break;
...
DoDiff(const char *file1, const char *file2)
{
  ...
  if((len2=StrGetLength(file2)) > MAX_STR_LEN){
    fprintf(stderr,"Error: argument is too long\n");
    return FALSE;
  }
  FILE *fp1,*fp2;
  ...
  while (fgets(buf1,sizeof(buf1),fp1)){
    if(fgets(buf2,sizeof(buf2),fp2)==0){
      fprintf(stderr,"Error: [%s] ends early at line %d\n",file2,line_cnt-1);
      return FALSE; 
    }
    if((len3=StrGetLength(buf1)) > MAX_STR_LEN){
      fprintf(stderr,"Error: input line [%s] is too long\n",file1);
      return FALSE;
    }
    if((len4=StrGetLength(buf2)) > MAX_STR_LEN){
      fprintf(stderr,"Error: input line [%s] is too long\n",file2);
      return FALSE;
    }
    if(StrCompare(buf1,buf2)!=0){
      printf("%s@%d:%s%s@%d:%s",file1,line_cnt,buf1,file2,line_cnt,buf2);
    }
    line_cnt++;
      }//end while
  if(fgets(buf2,sizeof(buf2),fp2)!=0){
